...
struct OpenRouterStatsDetailView: View {
    ...
    var body: some View { ScrollView { VStack(alignment: .leading, spacing: 16) { detailSection("OpenRouter", icon: "chart.bar.fill") { row("Сьогодні", money(stats.today.cost)); row("7 днів", money(stats.last7Days.cost)); row("30 днів", money(stats.last30Days.cost)); row("Запити за 30 днів", "\(stats.last30Days.requests)"); row("Вхідні токени", "\(stats.last30Days.inputTokens ?? 0)"); row("Вихідні токени", "\(stats.last30Days.outputTokens ?? 0)"); row("Баланс", stats.balance.map(money) ?? "Недоступний") } ; if !stats.topModels.isEmpty { detailSection("Моделі за 30 днів", icon: "list.number") { ForEach(stats.topModels) { model in row(model.model, money(model.cost)) } } }; detailSection("Джерело й оновлення", icon: "clock") { row("Джерело", "Локальний журнал Orbit"); if let date = stats.updatedAt { row("Оновлено", date.formatted(date: .abbreviated, time: .shortened)) }; Text("Баланс акаунта не підключений. Значення показують лише записані Orbit виклики.").font(.caption).foregroundStyle(.secondary) } }.padding(16) }.background(Color(uiColor: .systemGroupedBackground)).navigationTitle("OpenRouter") }
